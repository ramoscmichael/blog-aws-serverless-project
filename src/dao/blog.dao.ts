...
export const SELECT_COUNT_BLOG_POSTS = `SELECT count(p.id) FROM blog.post p WHERE p.user_id = :user_id`;
...
export class BlogDao {
    ...
    async getBlogPosts(conn: DBConnection, offset: number = 0, limit: number = 25, user_id: number): Promise<Post[]> {
        const res = await conn.query(SELECT_BLOG_POSTS, {user_id, offset, limit});
        return res.results.map((row: any) => this.toBlogPost(row));
    }
